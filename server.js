...
const APP_ID_LIVE  = '33iOIOUp9C72NdWd7IwpT';
const CLIENT_ID_LIVE = '019e1ee0-15d2-7645-8570-8c9db54820d3';
...
function createBot(id) {
  ...
  return {
    ...
    cfg: {
      ...
      apiToken:'', patToken:'',    // apiToken = demo legacy token; patToken = live PAT
    ...
    // ── TWO SEPARATE ZONE ARRAYS ──────────────────
    doNotTradeZones:[],   // Bot skips projected levels inside — no trading at all
    htfZones:[],          // Bot scans inside but pauses on uptrend structure forming
    // ─────────────────────────────────────────────
    htfZonePaused:false,
...
// ── ZONE HELPERS ─────────────────────────────────
// Do Not Trade Zone: level is skipped entirely
...
// HTF Zone: price proximity check for uptrend detection
...
function computeProjectedLevels(b,struct,tradedLevels) {
  ...
  // ONE next level per structure — skips Do Not Trade zones and ignored levels
...
// Uses htfZones only — completely separate from doNotTradeZones
...
function connectDeriv(b) {
  ...
  log(b,`🔌 Connecting to Deriv [${isLive?'LIVE':'DEMO'}]...`);
  setStatus(b,'connecting','CONNECTING');

  // Demo uses legacy WS endpoint; Live uses PAT endpoint
  const wsUrl = isLive
    ? `wss://green.derivws.com/websockets/v3?app_id=${APP_ID_LIVE}`
    : `wss://ws.binaryws.com/websockets/v3?app_id=${APP_ID_DEMO}`;
  ...
  b.derivWs.on('open',()=>{
    log(b,'🔗 Authorizing...');
    if(isLive){
      // PAT authorization for live account
      b.derivWs.send(JSON.stringify({
        authorize: b.cfg.patToken,
        client_id: CLIENT_ID_LIVE,
      }));
    } else {
      // Legacy token for demo
...
dashWss.on('connection',(ws)=>{
  ...
  ws.on('message',(raw)=>{
    ...
    if(msg.type==='start'){
      ...
      const isLive = b.cfg.accountType === 'live';
      if(isLive && !b.cfg.patToken){ ws.send(JSON.stringify({type:'error',id:b.id,msg:'No PAT token for live account'})); return; }
      if(!isLive && !b.cfg.apiToken){ ws.send(JSON.stringify({type:'error',id:b.id,msg:'No API token for demo account'})); return; }
