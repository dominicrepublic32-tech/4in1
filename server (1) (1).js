...
const APP_ID_LEGACY = 1089;                                    // demo — legacy API
const CLIENT_ID_PAT = '019e1ee0-15d2-7645-8570-8c9db54820d3'; // live — PAT API
...
function createBot(id) {
  ...
  return {
    id,
    cfg: {
      apiToken:'', patToken:'', accountType:'demo',
...
function checkHTFZoneUptrend(b) {
  ...
  const nearZone=b.htfZones.find(z=>{
    const lo=Math.min(z.a,z.b),hi=Math.max(z.a,z.b);
...
function connectDeriv(b) {
  ...
  const appId = b.cfg.accountType==='live' ? CLIENT_ID_PAT : APP_ID_LEGACY;
  b.derivWs=new WebSocket(`wss://ws.binaryws.com/websockets/v3?app_id=${appId}`);

  b.derivWs.on('open',()=>{
    log(b,`🔗 Authorizing [${b.cfg.accountType==='live'?'LIVE/PAT':'DEMO/Legacy'}]...`);
    const token = b.cfg.accountType==='live' ? b.cfg.patToken : b.cfg.apiToken;
    b.derivWs.send(JSON.stringify({authorize:token}));
  });
...
dashWss.on('connection',(ws)=>{
  ...
  ws.on('message',(raw)=>{
    ...
    if(msg.type==='start'){
      ...
      const token = b.cfg.accountType==='live' ? b.cfg.patToken : b.cfg.apiToken;
      if(!token){ws.send(JSON.stringify({type:'error',id:b.id,msg:`No ${b.cfg.accountType==='live'?'PAT':'API'} token`}));return;}
